...
function example() {
  ...
  return gulp.src('')
    ...
    .pipe(webpackStream({
      ...
      if (firstBuild) {
        $.livereload.listen({port: 35730, host: 'localhost', start: true});
...
// Set up a livereload environment for the example file `test/example.html`
gulp.task('example', ['lint', 'clean-tmp'], example);
